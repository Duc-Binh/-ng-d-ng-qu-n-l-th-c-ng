...
// xử lý sự kiện click nut delete trên bảng
...
//xử lý sự kiện click nút edit trên bảng
...
$("#submit-btn").click(function () {
  ...
  if (form === "create") {
    ...
    if (!petId.val()) {
      Swal.fire({
        icon: "error",
        html: `<p>Enter pet Id !</p>`,
        showConfirmButton: false,
        timer: 1500,
      });
    }
    if (validate && petId.val()) {
      $("#tbody").append(
        `<tr>
...
// timf kiếm pet khỏe mạnh(đã tiêm vacxin, tẩy giun, khử trùng)
$("#healthy-btn").click(function () {
  const petFilter = pet.filter(
...
$("#modal-search-pet").on("hidden.bs.modal", () => {
  $("#search-pet").val("");
});
